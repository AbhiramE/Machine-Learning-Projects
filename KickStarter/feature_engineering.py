...
def add_categories():
    ...
    pd.DataFrame(new_matrix).to_csv('final_t.csv', sep=",")


def add_features_and_drop_columns():
    df = pd.read_csv('final_t.csv')
    ...
    df = df[['project_id', 'name', 'desc', 'goal', 'disable_communication', 'country', 'state_changed_at',
             'created_at', 'category', 'duration<7', 'duration<15', 'duration<25', 'duration<35',
             'duration<45', 'duration<60', 'state_changed_before', 'final_status']]

    header = ['project_id', 'name', 'desc', 'goal', 'disable_communication', 'country', 'state_changed_at',
              'created_at', 'category', 'duration<7', 'duration<15', 'duration<25', 'duration<35',
              'duration<45', 'duration<60', 'state_changed_before', 'final_status']
    df.to_csv('new_train.csv', sep=',', columns=header)
